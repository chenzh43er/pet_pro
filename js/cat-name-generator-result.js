...
const LOADING_IMG = ASSETS.loadingCat;
const NAMES_FILE = `${ROOT}data/names/cat-names.json`;
...
async function fetchNameList(category, gender) {
  try {
...
function renderAside() {
  ...
  document.getElementById('aside-meaning-list').innerHTML = meanings.map(([value, label]) => (
    `<li class="name-meaning-item"><a href="${ROOT}pet-name.html?type=cat&meaning=${value}">${label}</a></li>`
  )).join('');

  document.getElementById('aside-origin-list').innerHTML = origins.map(([value, label]) => (
    `<li class="name-origin-item"><a href="${ROOT}pet-name.html?type=cat&origin=${value}">${label}</a></li>`
  )).join('');
}
...
async function renderRelatedPets() {
  ...
  try {
    const { data: petsData } = await fetchPets({ type: 'cat', limit: 6, offset: 0 });
...
async function init() {
  ...
  const items = await fetchNameList(category, gender);
